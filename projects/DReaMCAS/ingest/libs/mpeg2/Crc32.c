...
/*
 * $(@)Crc32.cc $Revision: 1.1.1.1 $ $Date: 2006/04/17 22:47:31 $
 * 
 * Copyright 2006 Sun Microsystems, Inc. All Rights Reserved.
 */

/**
 * The CRC routine is Mike Demoney's implementation.
 */
...
u32 crc32( const u8 *datap, int len )
{
    u32 crcSum = 0xffffffff;
    
    int buflen = len;
    const u8 *bufdatap = datap;
    /*
     * Table-based calculation code
     */
    while (len-- > 0) {

	crcSum = (crcSum >> 8) ^ crc32Table[(crcSum ^ *datap++) & 0xff];
    }
